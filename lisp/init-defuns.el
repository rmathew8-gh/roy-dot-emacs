...
(defun gf
  ...
  (interactive "p")
  (let* ((template "find %s %s -prune -o %s -type f -print0 | xargs -0 grep -in -e " ) 
         (prunes
          "\\( -name \".git\" -o -name \"ext-2.2\" -o -name \"Twisted*\" -o -name \"slave*\" -o -name \"external*\" \\)") 
         (specifics
          "\\( -name \"*.py\" -o -name \"*.html\" -o -name \"*.js\" -o -name \"*.txt\" -o -name \"*.pt\" -o -name \"*.zcml\" \\) ") 
         (big (format template "*" prunes specifics )) 
         (small (format template "."  "-name \".git\" " "" "")) 
         (command (cond ((eq 1 arg) small) 
                        (t big))) 
         (command (cons command (+ 1 (length command)))) 
         (command (read-from-minibuffer "Run find (like this): " command nil nil
                                        'grep-find-history))) 
    (grep-find command)))
...
(defun kustomer-code-search(arg ss) 
  ...
  (let ((cmd (if arg (concat "rg --files -i --iglob \"\*" ss "\*\"") 
...
(defun roy-grep-for-info(ss) 
  ...
  (grep (concat "grep -E \"" ss
                "\" -rni --include=\"*.nw\" --include=\"*.org\" ./Noweb-Files/Literate-Programming ./Noweb-Files.Personal/Literate-Programming")))
...
(defun roy-table-to-csv() 
  (interactive) 
  (save-excursion (set-mark (re-search-forward "</table>" nil t)) 
                  (re-search-backward "<table>" nil t) 
                  (shell-command-on-region (region-beginning) 
                                           (region-end) "table-to-csv.sh" 
                                           :replace t)))

(defun roy/org-strip-properties
    (&optional 
     beg
     end) 
  (interactive "r") 
...
(defun slu(args) 
  ...
  (let ((beg (if mark-active (region-beginning) 1)) 
